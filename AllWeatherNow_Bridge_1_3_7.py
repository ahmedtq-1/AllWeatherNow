...
def fetch_all_weather(lat, lon):
    ...
    try:
        ...
        # FIXED: Restricted dust storm classification strictly to actual WMO weather codes, ignoring erratic dust mass numerical spikes.
        ...
        # PM2.5 and humidity factor remain active
        pm25_ext = (pm25 * 0.003) * f_rh
        smoke_ext = (max(0.0, co - 200.0) * 0.00002) + (max(0.0, so2 - 10.0) * 0.0001)
        
        # FIXED: Removed `pm10_ext` and `dust_ext` entirely so Open-Meteo model dust artifacts do not degrade visibility.

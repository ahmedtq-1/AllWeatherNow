...
-- Integrated with Python Live Atmospheric Bridge & UDP Socket Receiving
...
if socket_ok then
    ...
    udp_server:setsockname("127.0.0.1", 5555)
    udp_server:settimeout(0) -- Non-blocking socket receiving
end
...
-- Parse Weather Data Helper (Shared between File Reader and UDP Receiver)
local function parse_and_apply_weather_data(data_string)
...
-- UDP Socket Receiver (Zero Disk I/O Real-time Streaming)
function update_weather_bridge_udp()
    if not udp_server then return false end
    ...
    while true do
        ...
        received_any = true
        parse_and_apply_weather_data(data)
    end

    if received_any then
        bridge_update_count = bridge_update_count + 1
        bridge_status = "Connected via UDP (Updates: " .. bridge_update_count .. ")"
        return true
    end
    return false
end
...
function update_weather_bridge()
    ...
    if file then
        ...
        parse_and_apply_weather_data(content)
        bridge_update_count = bridge_update_count + 1
        bridge_status = "Connected via File (Updates: " .. bridge_update_count .. ")"
    else
        bridge_status = "Error: all_weather_data.txt missing & UDP inactive"
    end
end
...
function update_nearest_airport_cache()
    ...
    if file then
        cached_nearest_airport = file:read("*a") or "Unknown"
        file:close()
    else
...
function process_seamless_frame()
    ...
    local alpha = math.min(1.0, dt * 0.4)
    ...
    -- Atmospheric Wind Matrix, Thermals, CAT & Shear
    if region_wind_alt then
        local ground_speed = d_groundspeed and XPLMGetDataf(d_groundspeed) or 100.0
        local cape_shear_add = 0.12 * math.sqrt(2.0 * math.max(0.0, target_env.cape or 0.0)) * math.sin(elapsed_time * 2.3)
        local wb_shear_add = calculate_wetbulb_downdraft()
        ...
        for i = 0, 12 do
            ...
            target_modulated_turb = math.max(target_modulated_turb, math.min(1.0, cat_turb + thermal_lift))
...
function atmosphere_tick()
    ...
    -- Receive live weather packets via UDP (instant, zero disk I/O)
    local udp_received = update_weather_bridge_udp()

    -- Fallback to file reading every 3s if UDP hasn't streamed or as backup
    if not udp_received and (os.clock() > last_update + 3) then
        export_aircraft_location()
        update_weather_bridge()
        update_nearest_airport_cache()
        last_update = os.clock()
    elseif udp_received and (os.clock() > last_update + 3) then
        export_aircraft_location()
        update_nearest_airport_cache()
        last_update = os.clock()
    end
...
function draw_weather_osd()
    ...
    draw_string(30, 600, "Nearest Airport: " .. cached_nearest_airport, "yellow")
end
